...
function previewText(value?: string) {
    return (value || "No prompt preview available yet.")
        .replace(/<br\s*\/?>/gi, "\n")
        .replace(/<\/p>/gi, "\n")
        .replace(/<[^>]+>/g, "")
        .replace(/\*\*([^*]+)\*\*/g, "$1")
...
export default function SystemDesignQuestionBankPage() {
    ...
    const [datasetQuestions, setDatasetQuestions] = useState<SystemDesignQuestion[]>([]);
    const [datasetTotal, setDatasetTotal] = useState(0);
    const [datasetSearch, setDatasetSearch] = useState("");
    const [datasetDifficulty, setDatasetDifficulty] = useState<Difficulty | null>(null);
    ...
    const loadQuestions = useCallback(async () => {
        ...
        setError(null);
        try {
            const payload = await api.get<QuestionsResponse>("/companies/question-bank/system-design?limit=100", session.access_token);
            setQuestions(payload.questions || []);
        } catch (err) {
            setError(err instanceof ApiError ? err.message : "Failed to load system design questions.");
        } finally {
            setLoading(false);
        }
    ...
    const loadDatasetQuestions = useCallback(async () => {
        ...
        try {
            ...
            if (datasetDifficulty) params.set("difficulty", datasetDifficulty);
            const payload = await api.get<QuestionsResponse>(`/companies/question-bank/system-design/dataset?${params.toString()}`, session.access_token);
            setDatasetQuestions(payload.questions || []);
            setDatasetTotal(payload.pagination?.total || 0);
        } catch (err) {
            setDatasetError(err instanceof ApiError ? err.message : "Failed to load system design dataset.");
        } finally {
            setDatasetLoading(false);
        }
    }, [datasetDifficulty, datasetSearch, session?.access_token]);
    ...
    async function importDatasetQuestion(questionId: string) {
        ...
        try {
            const result = await api.post<{ imported?: number; skipped?: number; missing?: number }>(
                "/companies/question-bank/system-design/import",
                { questionIds: [questionId] },
                session.access_token
            );
            await Promise.all([loadQuestions(), loadDatasetQuestions()]);
            if (!result.imported && result.skipped) setDatasetError("This question is already available in your system design bank.");
            else if (!result.imported && result.missing) setDatasetError("This system design question could not be found in the dataset.");
        } catch (err) {
            setDatasetError(err instanceof ApiError ? err.message : "Failed to import system design question.");
        } finally {
    ...
    async function removeQuestion(questionId: string) {
        ...
        setError(null);
        try {
            await api.delete(`/companies/question-bank/system-design/${questionId}`, session.access_token);
            await Promise.all([loadQuestions(), datasetOpen ? loadDatasetQuestions() : Promise.resolve()]);
        } catch (err) {
            setError(err instanceof ApiError ? err.message : "Failed to remove system design question.");
        } finally {
    ...
    const visibleQuestions = useMemo(() => questions.filter((question) => {
        const matchesSearch =
            ...
            previewText(question.problemStatement).toLowerCase().includes(normalizedSearch) ||
            question.tags?.some((tag) => tag.toLowerCase().includes(normalizedSearch));
        const matchesDifficulty = !selectedDifficulty || question.difficulty === selectedDifficulty;
        return matchesSearch && matchesDifficulty;
    }), [normalizedSearch, questions, selectedDifficulty]);
            ...
            <section className="px-4 sm:px-6 lg:px-10">
                <div className="flex flex-col gap-5">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                        <label className="relative block w-full sm:w-[360px]">
                            ...
                            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search questions" className="h-14 w-full rounded-full border border-slate-200 bg-white pl-14 pr-4 text-sm font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-primary focus:ring-4 focus:ring-primary/10 dark:border-lc-border dark:bg-lc-surface dark:text-white" />
                        </label>
                        ...
                        {!loading && questions.length > 0 && (
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <Link href="/question-bank/system-design/create-question" className="inline-flex h-12 items-center gap-2 rounded-full bg-slate-100 px-5 text-sm font-extrabold text-slate-700 transition hover:bg-slate-200 dark:bg-lc-elevated dark:text-slate-200 dark:hover:bg-lc-hover dark:hover:text-white">
                    ...
                    <div className="flex flex-wrap gap-3">
                        <button type="button" onClick={() => setSelectedDifficulty(null)} className={`inline-flex h-11 items-center rounded-full px-5 text-sm font-extrabold transition ${!selectedDifficulty ? "bg-primary text-white" : "bg-slate-100 text-slate-700 hover:bg-primary/10 hover:text-primary dark:bg-lc-surface dark:text-slate-200 dark:hover:bg-lc-hover dark:hover:text-white"}`}>All</button>
                        {difficulties.map((difficulty) => (
                            <button key={difficulty} type="button" onClick={() => setSelectedDifficulty(selectedDifficulty === difficulty ? null : difficulty)} className={`inline-flex h-11 items-center rounded-full px-5 text-sm font-extrabold transition ${selectedDifficulty === difficulty ? "bg-primary text-white" : "bg-slate-100 text-slate-700 hover:bg-primary/10 hover:text-primary dark:bg-lc-surface dark:text-slate-200 dark:hover:bg-lc-hover dark:hover:text-white"}`}>
                                {difficulty}
                            </button>
                        ))}
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="mt-12 rounded-2xl bg-white p-8 text-center shadow-sm dark:bg-lc-surface">
                        <h2 className="font-nunito text-2xl font-extrabold text-slate-950 dark:text-white">No system design questions yet</h2>
                        <p className="mx-auto mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-500 dark:text-slate-400">Add architecture prompts from the dataset or create a company-owned system design prompt.</p>
                        <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                            ...
                            <Link href="/question-bank/system-design/create-question" className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-slate-100 px-5 text-sm font-extrabold text-slate-700 transition hover:bg-slate-200 dark:bg-lc-elevated dark:text-slate-200 dark:hover:bg-lc-hover dark:hover:text-white">
                                <span className="material-symbols-outlined text-[20px]">add</span>
                    ...
                    <div className="mt-8 overflow-hidden rounded-xl bg-white shadow-sm dark:bg-lc-surface">
                        {visibleQuestions.map((question) => (
                            <div key={question.id} className="flex flex-col gap-4 border-b border-slate-100 px-6 py-5 last:border-b-0 dark:border-lc-border lg:flex-row lg:items-center lg:justify-between">
                                <Link href={`/question-bank/system-design/${question.id}`} className="min-w-0 flex-1 transition hover:text-primary">
                                    <div className="flex flex-wrap items-center gap-3">
                                        <span className={`rounded-full px-3 py-1 text-xs font-extrabold ${difficultyClass[question.difficulty]}`}>{question.difficulty}</span>
                                        <h2 className="font-nunito text-lg font-extrabold text-slate-950 dark:text-white">{question.title}</h2>
                                    </div>
                                    <p className="mt-2 line-clamp-2 text-sm font-semibold leading-6 text-slate-500 dark:text-slate-400">{previewText(question.problemStatement)}</p>
                                </Link>
                    ...
                    <div className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 px-4 py-5 backdrop-blur dark:border-lc-border dark:bg-lc-bg/95 sm:px-6 lg:px-10">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h2 className="font-nunito text-3xl font-extrabold text-slate-950 dark:text-white">Add System Design Questions</h2>
                        ...
                        <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <label className="relative block w-full lg:w-[420px]">
                                <span className="material-symbols-outlined pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[24px] text-slate-400">search</span>
                                <input value={datasetSearch} onChange={(event) => setDatasetSearch(event.target.value)} placeholder="Search dataset questions" className="h-14 w-full rounded-full border border-slate-200 bg-white pl-14 pr-4 text-sm font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-primary focus:ring-4 focus:ring-primary/10 dark:border-lc-border dark:bg-lc-surface dark:text-white" />
                            </label>
                            <div className="flex flex-wrap gap-2">
                                <button type="button" onClick={() => setDatasetDifficulty(null)} className={`h-11 rounded-full px-4 text-sm font-extrabold transition ${!datasetDifficulty ? "bg-primary text-white" : "bg-slate-100 text-slate-600 dark:bg-lc-surface dark:text-slate-300 dark:hover:bg-lc-hover dark:hover:text-white"}`}>All</button>
                                {difficulties.map((difficulty) => (
                                    <button key={difficulty} type="button" onClick={() => setDatasetDifficulty(difficulty)} className={`h-11 rounded-full px-4 text-sm font-extrabold transition ${datasetDifficulty === difficulty ? "bg-primary text-white" : "bg-slate-100 text-slate-600 dark:bg-lc-surface dark:text-slate-300 dark:hover:bg-lc-hover dark:hover:text-white"}`}>{difficulty}</button>
                                ))}
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="divide-y divide-slate-100 overflow-hidden rounded-xl bg-white shadow-sm dark:divide-lc-border dark:bg-lc-surface">
                                {datasetQuestions.map((question) => (
                                    <div key={question.id} className="grid gap-4 px-6 py-5 lg:grid-cols-[1fr_auto] lg:items-center">
                                        <Link href={`/question-bank/system-design/${question.id}?source=dataset`} className="min-w-0 pr-2 transition hover:text-primary">
                                            <div className="flex flex-wrap items-center gap-3">
                                                <span className={`rounded-full px-3 py-1 text-xs font-extrabold ${difficultyClass[question.difficulty]}`}>{question.difficulty}</span>
                                                <h3 className="font-nunito text-lg font-extrabold text-slate-950 dark:text-white">{question.title}</h3>
                                            </div>
                                            <p className="mt-2 line-clamp-2 text-sm font-semibold leading-6 text-slate-500 dark:text-slate-400">{previewText(question.problemStatement)}</p>
                                        </Link>
                                ...
                                {!datasetQuestions.length && <div className="p-8 text-center text-sm font-semibold text-slate-500 dark:text-slate-400">No dataset questions found.</div>}
                            </div>
                        )}
                    </div>
                </div>
