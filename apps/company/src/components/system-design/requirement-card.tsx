...
export default function RequirementCard({ title, placeholder, value, onChange }: Props) {
            ...
            try {
                ...
                recognition.onend = () => {
                    ...
                    if (listening && !stopRequestedRef.current) {
                        setTimeout(() => {
                            if (listening && !stopRequestedRef.current) {
                                try { recognitionRef.current?.start(); } catch { startMic(); }
                            }
                        }, 300);
                    } else {
                        setListening(false);
    ...
    return (
        <div className={`flex flex-col h-full bg-white dark:bg-[#1f1f1f] rounded-xl shadow-sm overflow-hidden transition-shadow ${listening ? "ring-2 ring-blue-300 dark:ring-blue-700/60" : ""}`}>
            <div className="flex items-center justify-between px-4 py-3">
                ...
                <button
                    ...
                    title={unsupported ? "Speech recognition not supported" : listening ? "Stop dictation" : "Dictate"}
                    className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${listening ? "bg-blue-600 text-white shadow-md shadow-blue-500/40" : "bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/60"}`}
                >
                    {listening && <span className="absolute -inset-0.5 rounded-full bg-blue-400/30 animate-ping" />}
                    <span className="material-symbols-outlined text-[16px] relative">{listening ? "stop_circle" : "mic"}</span>
            ...
            <div className="flex-1 relative px-1 pb-1">
                <textarea
                    value={value}
                    onChange={(e) => { valueRef.current = e.target.value; onChange(e.target.value); }}
