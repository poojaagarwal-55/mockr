...
import { AlertTriangle, ArrowLeft, ArrowRight, CheckCircle2, Clock, Lock, ShieldCheck } from "lucide-react";
...
function mcqRoundCacheKey(contestId: string) {
  return `contest-mcq-round-cache:${contestId}`;
}

function readMcqRoundCache(contestId: string): McqRoundCache | null {
  if (typeof window === "undefined" || !contestId) return null;
  try {
    const raw = window.sessionStorage.getItem(mcqRoundCacheKey(contestId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as McqRoundCache;
...
function writeMcqRoundCache(contestId: string, cache: Omit<McqRoundCache, "savedAt">) {
  if (typeof window === "undefined" || !contestId || cache.questions.length === 0) return;
  try {
    window.sessionStorage.setItem(
      mcqRoundCacheKey(contestId),
...
export default function ContestMcqRoundPage() {
  ...
  const submitted = rounds?.rounds?.mcq?.status === "submitted" || rounds?.rounds?.mcq?.status === "auto_submitted";
  const serverMcqWarningCount = Number(rounds?.rounds?.mcq?.warningCount ?? 0);
  const serverTotalWarningCount = serverMcqWarningCount + Number(rounds?.rounds?.dsa?.warningCount ?? 0);
  const {
    ...
    contestId,
    roundType: "mcq",
    submitted: submitted || autoSubmitted,
    getToken,
  });

  const hydrateFromCache = useCallback(() => {
    const cached = readMcqRoundCache(contestId);
  ...
  const load = useCallback(async (options?: { silent?: boolean; startRound?: boolean }) => {
    ...
    try {
      ...
      if (options?.startRound !== false) {
        await fetch(`${CONTEST_API}/contests/${contestId}/rounds/mcq/start`, {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        });
      }

      const [contestResponse, questionsResponse, roundsResponse] = await Promise.all([
      ...
      if (!questionsResponse.ok) throw new Error(questionsData.message || "Failed to load MCQ questions");
      const nextQuestions = Array.isArray(questionsData.questions) ? questionsData.questions : [];
      if (nextQuestions.length === 0) throw new Error("MCQ questions are still loading. Please wait a moment.");
      ...
      if (!silent) {
        setError(err instanceof Error ? err.message : "Failed to load MCQ round");
      }
    } finally {
  ...
  // Hide the app sidebar/nav while inside the live MCQ round.
  useEffect(() => {
  ...
  useEffect(() => {
    syncWarningCount(serverMcqWarningCount, serverTotalWarningCount);
  }, [serverMcqWarningCount, serverTotalWarningCount, syncWarningCount]);

  const sequential = rounds?.settings?.mcqSequential === true;
  const isMcqOnlyContest = rounds?.settings?.roundFlow === "mcq_only";
  const nextAllowedOrder = rounds?.rounds?.mcq?.nextAllowedOrder ?? 0;
  const mcqSubmittedCount = rounds?.rounds?.mcq?.submittedCount ?? questions.filter((question) => question.status === "submitted").length;
  const mcqQuestionCount = rounds?.rounds?.mcq?.questionCount ?? questions.length;
  const mcqPossibleScore = questions.reduce((sum, question) => sum + Math.max(0, Number(question.points || 0)), 0);
  ...
  // Mixed contests: once the MCQ round itself is submitted, block re-entry to the round list.
  useEffect(() => {
    if (!contestId || isMcqOnlyContest) return;
    if (submitted && !submittedHereRef.current && !showSubmitSuccess) {
      router.replace(`/contests/${contestId}`);
    }
  }, [contestId, isMcqOnlyContest, submitted, showSubmitSuccess, router]);
  ...
  const submitRound = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || "Failed to submit MCQ round");
      if (isMcqOnlyContest) {
      ...
      setQuestions((current) => current.map((question) => ({ ...question, status: "submitted" })));
      setRounds((current) => current ? {
        ...
        rounds: {
          ...
          mcq: {
            ...current.rounds.mcq,
            status: "submitted",
            submittedCount: current.rounds.mcq.questionCount,
            nextAllowedOrder: current.rounds.mcq.questionCount,
          },
          dsa: {
            ...current.rounds.dsa,
            unlocked: true,
      ...
      setError(err instanceof Error ? err.message : "Failed to submit MCQ round");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleBackToContest = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${CONTEST_API}/contests/${contestId}/rounds/mcq/leave-status`, {
        headers: { Authorization: `Bearer ${token}` },
        cache: "no-store",
      });
      const data = await response.json().catch(() => ({}));
      if (response.ok && data?.canLeave === false) {
        setLeaveBlockedMessage(data.message || "Submit the current MCQ before leaving this round.");
        return;
      }
      router.push(isMcqOnlyContest ? "/contests" : `/contests/${contestId}`);
    } catch {
      setLeaveBlockedMessage("We could not verify your round state. Please stay on this page and try again.");
    } finally {
      setCheckingLeave(false);
    }
  }, [contestId, getToken, isMcqOnlyContest, router]);
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] px-4 py-6 text-slate-950 dark:bg-lc-bg dark:text-white sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
        <div className="flex flex-col gap-4 pb-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-nunito text-3xl font-black">MCQ Test</h1>
              <p className="mt-2 text-sm font-bold text-slate-500 dark:text-slate-400">
                Score and correctness are hidden in this round.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3 sm:justify-end">
            ...
            <Button
              onClick={submitRound}
              disabled={loading || submitting || submitted || questions.length === 0}
            ...
            >
              {submitted ? "Submitted" : submitting ? "Submitting..." : "Finish Your MCQ Attempt"}
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-6 border-b border-slate-200 py-6 text-left dark:border-white/10 sm:grid-cols-4">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">MCQ Round Score</p>
            <p className="mt-1 inline-flex items-center gap-2 text-base font-black text-slate-950 dark:text-white">
              ...
              <span>/ {mcqPossibleScore}</span>
            </p>
          </div>
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Solved</p>
            <p className="mt-1 text-base font-black text-slate-950 dark:text-white">{mcqSubmittedCount} / {mcqQuestionCount}</p>
          </div>
          <div>
          ...
          <section className="min-h-0 flex-1 overflow-y-auto pt-6">
            <div className="space-y-2">
              {questions.map((question, index) => {
                ...
                const phaseOrder = question.phaseOrder ?? index;
                const lockedBySequence = sequential && phaseOrder !== nextAllowedOrder && !isSubmitted;
                const locked = submitted || isSubmitted || lockedBySequence;
                return (
                  <button
                    key={question.id}
                    type="button"
                    ...
                    onClick={() => {
                      markIntegrityStarted();
                      router.push(`/contests/${contestId}/mcq/${question.id}`);
                    }}
                    className={`group grid w-full grid-cols-[2.25rem_minmax(0,1fr)_auto_auto] items-center gap-4 px-2 py-5 text-left transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-70 dark:hover:bg-lc-hover sm:px-4 ${isSubmitted ? "bg-emerald-50/70 dark:bg-emerald-400/10" : ""}`}
                  >
                    ...
                    <span className="text-right text-xs font-black text-slate-950 dark:text-white">
                      {question.points}
                      <span className="mt-0.5 block font-bold text-slate-500 dark:text-slate-400">Marks</span>
                    </span>
                    <span className="grid h-9 w-9 place-items-center rounded-full border border-slate-300 bg-white text-slate-600 group-hover:border-primary group-hover:text-primary dark:border-white/20 dark:bg-white/5">
                      {isSubmitted ? <CheckCircle2 className="h-4 w-4" /> : lockedBySequence || submitted ? <Lock className="h-4 w-4" /> : <ArrowRight className="h-4 w-4" />}
                    </span>
                  </button>
                );
              })}
      ...
      {showSubmitSuccess && (
        <div className="fixed inset-0 z-[10000] grid place-items-center bg-slate-950/45 p-4 dark:bg-black/65">
          <button
            ...
            onClick={() => router.replace(`/contests/${contestId}`)}
          />
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="mcq-submit-popup-title"
            aria-describedby="mcq-submit-popup-message"
          ...
          >
            <h2 id="mcq-submit-popup-title" className="font-nunito text-lg font-extrabold">
              Submission recorded
            </h2>
            <p id="mcq-submit-popup-message" className="mt-2 text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">
            ...
            <div className="mt-5 flex justify-end">
              <Button
                type="button"
                onClick={() => router.replace(`/contests/${contestId}`)}
                className="h-10 rounded-md px-5 font-extrabold"
              >
                OK
              </Button>
            </div>
      ...
      {showFullscreenPrompt && !submitted && !autoSubmitted && (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">
                  MCQ round proctoring is active. Stay in fullscreen and keep this tab focused until you submit the round.
                </p>
              </div>
            </div>
