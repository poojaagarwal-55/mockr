...
function SQLQuestionRow({
    ...
    return (
        <>
            ...
            >
                {/* Selection Checkbox or Solved indicator */}
                <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center">
                    {isSelectionMode ? (
                        ...
                        <span className="material-symbols-outlined text-slate-300 dark:text-slate-600 text-[18px]">radio_button_unchecked</span>
                    )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2">
                        ...
                        <h3 className="text-slate-900 dark:text-white font-medium truncate">
                            {normalizeDisplayText(question.title)}
                        </h3>
                    </div>
                </div>
...
export default function SQLQuestionsPage() {
    useEffect(() => { document.title = "SQL Questions | Mockr"; }, []);
        ...
        const fetchQuestions = async () => {
            ...
            try {
                ...
                const res = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/ide/sql/questions?${params.toString()}`,
    ...
    // Recompute solvedCount from progress filtered to loaded SQL question IDs
    useEffect(() => {
        if (questions.length === 0) return;
        const sqlIds = new Set(questions.map(q => q.id));
        const count = Object.entries(progress).filter(
            ([id, p]) => sqlIds.has(id) && p?.status === "solved"
        ).length;
    ...
    // Infinite scroll
    useEffect(() => {
        const container = document.getElementById("sql-scroll-container");
        if (!container) return;
        const handleScroll = () => {
    ...
    const statusFiltered = questions.filter(q => {
        ...
        if (statusFilter === "Solved") return qProgress?.status === "solved";
        if (statusFilter === "Attempted") return qProgress?.status === "attempted";
        if (statusFilter === "Unsolved") return !qProgress || (qProgress.status !== "solved" && qProgress.status !== "attempted");
        return true;
    });
    ...
    const searchFiltered = normalizedSearch.length > 0
        ? statusFiltered.filter((q) => {
            if (q.title.toLowerCase().startsWith(normalizedSearch)) return true;
            if (q.description?.toLowerCase().startsWith(normalizedSearch)) return true;
    ...
    return (
        <div className="flex-1 overflow-hidden bg-white dark:bg-[#282828] flex flex-col">
            <PageHeader titleNode={<h1 className="text-[28px] font-bold text-slate-900 dark:text-white font-nunito tracking-[-0.02em]">SQL Questions</h1>} showBack={true} backUrl="/questions" />

            <div className="flex-1 flex overflow-hidden">
                <div className="flex-1 flex flex-col overflow-hidden">
                    <div id="sql-scroll-container" className="flex-1 overflow-y-auto flex flex-col">
                                ...
                                <div className="relative">
                                    ...
                                    {showFilterModal && (
                                        <>
                                            ...
                                            <div className="absolute top-full left-0 mt-2 bg-white dark:bg-[#282828] rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden z-50 min-w-[160px]">
                                                {["All", "Solved", "Unsolved", "Attempted"].map(status => (
                                                    <button
                                                        ...
                                                        className={`w-full flex items-center justify-between gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-left ${
                                                            statusFilter === status ? "bg-blue-50 dark:bg-blue-900/20" : ""
                                                        }`}
                                                    >
                                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{status}</span>
                                                        {statusFilter === status && <span className="material-symbols-outlined text-blue-600 dark:text-blue-400 text-[18px]">check</span>}
                                                    </button>
                                                ))}
                                            </div>
                                        </>
                                ...
                                <div className="flex-1 flex justify-end">
                                    <div className="flex items-center gap-2 mr-4">
                                        <div className="relative w-6 h-6">
                                            <svg className="w-6 h-6 transform -rotate-90" viewBox="0 0 36 36">
                                                ...
                                                <circle
                                                    ...
                                                    strokeDasharray={`${((solvedCount / (pagination.total || 1)) * 100.48).toFixed(2)} 100.48`}
                                        ...
                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                                            {solvedCount}/{pagination.total} Solved
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div className="flex-1 bg-white dark:bg-[#282828]">
                            {loading && currentPage === 1 ? (
                                ...
                                <>
                                    <div>
                                        {sortedQuestions.map((question, idx) => (
                                            <SQLQuestionRow
                                                ...
                                                question={question}
                                                idx={idx}
                                                onNavigate={() => router.push(`/questions/sql/solve?id=${question.id}`)}
                                    ...
                                    {/* Load more indicator */}
                                    <div id="sql-infinite-trigger" className="h-4 w-full" />
                                    {loading && currentPage > 1 && (
