...
type PreviewTestRunResponse = {
    compileOutput?: string;
    compile_output?: string;
    errorDetails?: string;
    status?: string;
    error?: string;
    results?: PreviewTestRunCaseResult[];
};

type NormalizedPreviewResult = {
    status: string;
    input?: string;
    expected?: string;
    actual?: string;
...
function normalizeHiddenTests(q: QuestionPayload): any[] {
    return Array.isArray(q.hiddenTestCases) ? q.hiddenTestCases : [];
}
...
function getExecutionStatusLabel(status: any, passed?: boolean): string {
    const normalized = String(status || "").trim().toUpperCase();
    if (passed || normalized === "AC" || normalized === "ACCEPTED") return "Accepted";
    if (normalized === "TLE" || normalized.includes("TIME LIMIT")) return "Time Limit Exceeded";
    if (normalized === "MLE" || normalized.includes("MEMORY LIMIT")) return "Memory Limit Exceeded";
    if (normalized === "CE" || normalized.includes("COMPIL")) return "Compilation Error";
    if (normalized === "RE" || normalized.includes("RUNTIME")) return "Runtime Error";
    if (normalized === "WA" || normalized.includes("WRONG")) return "Wrong Answer";
    if (normalized === "RUNNING") return "Running";
    if (normalized === "PENDING") return "Pending";
    return status ? String(status) : "Wrong Answer";
}

function isAcceptedResult(result: any): boolean {
    if (!result) return false;
    return getExecutionStatusLabel(result?.status, result?.passed) === "Accepted";
}

function isFailedResult(result: any): boolean {
    if (!result) return false;
    const label = getExecutionStatusLabel(result?.status, result?.passed);
    return !["Accepted", "Running", "Pending"].includes(label);
}

function getResultOutput(result: any): string {
    return result?.actual || result?.stderr || result?.compileOutput || result?.error || "(no output)";
}
...
function buildPythonClassSolutionAdapters(userCode: string): string {
    ...
    for (const line of lines) {
        ...
        methods.push({
            name: methodMatch[1] || "",
            params: (rawParams[0] === "self" || rawParams[0] === "cls" ? rawParams.slice(1) : rawParams).join(", "),
        });
    }

    return methods
        .filter((method) => method.name && !method.name.startsWith("__") && !new RegExp(`^def\\s+${method.name}\\s*\\(`, "m").test(userCode))
        .map((method) => {
            const args = splitParams(method.params).map((param, index) => extractSimpleArgName(param, `arg${index}`)).join(", ");
            return `def ${method.name}(${method.params}):\n    return Solution().${method.name}(${args})`;
        })
        .join("\n\n");
}
...
function buildJavaScriptClassSolutionAdapters(userCode: string): string {
    ...
    return methods
        .filter((method, index, list) =>
            list.findIndex((candidate) => candidate.name === method.name) === index &&
            !new RegExp(`^\\s*(?:async\\s+)?function\\s+${method.name}\\s*\\(`, "m").test(userCode)
        )
        .map((method) => {
            const args = splitParams(method.params).map((param, index) => extractSimpleArgName(param, `arg${index}`)).join(", ");
            ...
            return `${asyncPrefix}function ${method.name}(${method.params}) {\n  return ${awaitPrefix}${receiver}.${method.name}(${args});\n}`;
        })
        .join("\n\n");
}

function combinePreviewCodeWithWrapper(userCode: string, wrapperCode: string, language: string): string {
    if (!wrapperCode.trim()) return userCode;
    if (wrapperCode.includes("<USER_CODE>")) return wrapperCode.replace(/<USER_CODE>/g, userCode);

    const lang = (language || "").toLowerCase();
    if (lang === "java") {
        const importLines: string[] = [];
        const restLines: string[] = [];
        for (const line of wrapperCode.split("\n")) {
            const trimmed = line.trim();
            if (trimmed.startsWith("import ") || trimmed.startsWith("package ")) importLines.push(line);
    ...
    if (lang === "cpp" || lang === "c++") {
        ...
        for (const line of wrapperCode.split("\n")) {
            const trimmed = line.trim();
            if (trimmed.startsWith("#include") || trimmed.startsWith("using namespace") || trimmed.startsWith("using std::")) headerLines.push(line);
            else restLines.push(line);
        }
        return [headerLines.join("\n") || "#include <bits/stdc++.h>\nusing namespace std;", userCode, restLines.join("\n")]
...
function parseExampleText(raw: string): { inputStr: string | null; outputStr: string | null; explanation: string | null } {
    const inputM = raw.match(/\*{0,2}Input\s*:\*{0,2}\s*([\s\S]*?)(?=\n\s*\*{0,2}Output\s*:\*{0,2}|$)/i);
    const outputM = raw.match(/\*{0,2}Output\s*:\*{0,2}\s*([\s\S]*?)(?=\n\s*\*{0,2}Explanation\s*:\*{0,2}|$)/i);
    const explM = raw.match(/\*{0,2}Explanation\s*:\*{0,2}\s*([\s\S]*)$/i);
    return {
...
function DSAPreview({ q, backHref, isDark, onBack }: { q: QuestionPayload; backHref: string; isDark: boolean; onBack?: () => void }) {
    ...
    const hiddenTests = normalizeHiddenTests(q);
    ...
    const [tests, setTests] = useState<any[]>(sampleTests);
    ...
    const constraintItems = getRichConstraintItems(q.constraints);
    ...
    const runAllTests = async (mode: "run" | "submit" = "run") => {
        ...
        const testsToRun = isSubmit ? [...sampleTests, ...hiddenTests] : sampleTests;
        if (!q || testsToRun.length === 0) return;
        setTests(testsToRun);
        ...
        setResults(() => {
            ...
            testsToRun.forEach((t, idx) => (next[getTestResultKey(t, idx)] = { status: "Running" }));
            return next;
        });
        try {
            ...
            const wrapperCode =
                snippets[normalizedLanguage]?.wrapper_code ||
                snippets[language]?.wrapper_code ||
                snippets[language === "cpp" ? "c++" : language === "c++" ? "cpp" : language]?.wrapper_code ||
                "";
            const executableCode = combinePreviewCodeWithWrapper(code, wrapperCode, normalizedLanguage);
            const sampleCount = sampleTests.length;
            const payloadTests = testsToRun.map((test, index) => ({
                id: getTestResultKey(test, index),
                input: test.input ?? test.Input ?? "",
                expected: test.output ?? test.expectedOutput ?? test.expected_output ?? test.Output ?? "",
                type: index < sampleCount ? "sample" as const : "hidden" as const,
            }));

            const data = await api.post<PreviewTestRunResponse>(
                "/code/test-run",
                {
                    code: executableCode,
                    ...
                    testCases: payloadTests,
                    ...(q.judgeType === "custom" && q.checkerCode
                        ? { checker: { language: q.checkerLanguage || "cpp", code: q.checkerCode } }
                        : {}),
                },
                token
            );

            const compileOutput = data.compileOutput || data.compile_output || data.errorDetails;
            if (compileOutput || data.status === "CE") {
                setSubmissionResult({ status: "compile_error", message: "Compilation Error", errorDetails: compileOutput });
                setResults((prev) => {
                    const next = { ...prev };
                    testsToRun.forEach((t, idx) => (next[getTestResultKey(t, idx)] = { status: "Pending" }));
                    return next;
                });
                return;
            }
            if (data.error) throw new Error(data.error || "Code execution failed.");

            if (Array.isArray(data.results)) {
                const map: Record<string, NormalizedPreviewResult> = {};
                const normalizedResults = data.results.map((test, idx): NormalizedPreviewResult => {
                    const status = getExecutionStatusLabel(test.status, test.passed);
                    return {
                        status,
                        input: test.input,
                        expected: test.expected ?? test.expectedOutput,
                        actual: test.actual ?? test.actualOutput,
                        passed: status === "Accepted",
                        runtime: test.runtimeMs ?? test.runtime,
                        memory: test.memory,
                        type: test.type ?? payloadTests[idx]?.type,
                        stderr: test.stderr,
                        compileOutput: test.compileOutput,
                    };
                });
                normalizedResults.forEach((test, idx: number) => {
                    const id = getTestResultKey(testsToRun[idx], idx);
                    map[id] = test;
                });
                setResults(map);

                const sampleResults = normalizedResults.filter((test) => test.type !== "hidden");
                const hiddenResults = normalizedResults.filter((test) => test.type === "hidden");
                const samplePassed = sampleResults.filter(isAcceptedResult).length;
                const sampleTotal = sampleResults.length;
                const hiddenPassed = hiddenResults.filter(isAcceptedResult).length;
                const hiddenTotal = hiddenResults.length;
                const firstHiddenFailed = hiddenResults.find(isFailedResult);
                const allVisiblePassed = normalizedResults.length > 0 && normalizedResults.every(isAcceptedResult);

                if (isSubmit) {
                    const accepted = allVisiblePassed;
                    setSubmissionResult({
                        ...
                        hiddenPassed,
                        hiddenTotal,
                        hiddenFirstFailed: firstHiddenFailed
                            ? {
                                status: firstHiddenFailed.status,
                                input: firstHiddenFailed.input,
                                expectedOutput: firstHiddenFailed.expected,
                                actualOutput: firstHiddenFailed.actual,
                                stderr: firstHiddenFailed.stderr,
                                compileOutput: firstHiddenFailed.compileOutput,
                            }
                            : undefined,
                    });
                }
                    ...
                    <div className="h-full flex flex-col bg-white dark:bg-[#282828]">
                        ...
                        <div className="flex-1 overflow-auto p-6">
                            ...
                            <div className="mb-4 flex items-center gap-3">
                                ...
                                <span className="inline-block rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600 dark:bg-[#333] dark:text-slate-300">
                                    CPU {Math.round((q.timeLimit ?? 2) * 1000)} ms
                                </span>
                                <span className="inline-block rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600 dark:bg-[#333] dark:text-slate-300">
                                    {q.memoryLimit ?? 256} MB
                                </span>
                            </div>
                            ...
                            {activeTab === "description" && (
                                <div className="prose dark:prose-invert max-w-none">
                                    <div className="text-slate-700 dark:text-slate-100 leading-relaxed">
                                        <RichQuestionContent content={statement} />
                                    </div>
                                            ...
                                            <div className="mt-8">
                                                ...
                                                {cards.map((c) => (
                                                    <div key={c.idx} className="mb-6 p-4 bg-slate-50 dark:bg-[#1c160d] rounded-lg">
                                                        ...
                                                        {c.rawText && (
                                                            <div className="rounded bg-white p-3 dark:bg-[#282828]">
                                                                <RichQuestionContent content={c.rawText} compact />
                                                            </div>
                                                        )}
                                                        {c.inputStr && <div className="mb-3"><div className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Input:</div><div className="p-3 bg-white dark:bg-[#282828] rounded"><QuestionExampleValue value={c.inputStr} /></div></div>}
                                                        {c.outputStr && <div className="mb-3"><div className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Output:</div><div className="p-3 bg-white dark:bg-[#282828] rounded"><QuestionExampleValue value={c.outputStr} /></div></div>}
                                                        {c.explanation && <div><div className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Explanation:</div><RichQuestionContent content={c.explanation} compact /></div>}
                                                    </div>
                                                ))}
                                            </div>
                                        );
                                        ...
                                        <div className="mt-8">
                                            ...
                                            <div className="max-w-full min-w-0 overflow-hidden p-4 bg-slate-50 dark:bg-[#1c160d] rounded-lg">
                                                <div className="max-w-full min-w-0 space-y-2 text-sm text-slate-700 dark:text-slate-100">
                                                    {constraintItems.map((constraint: string, i: number) => (
                                                        <div key={i} className="max-w-full min-w-0 rounded-md bg-white/70 px-3 py-2 leading-relaxed break-words [overflow-wrap:anywhere] dark:bg-[#282828]">
                                                    ...
                                                    {expandedSolution === "bruteForce" && (
                                                        <div className="p-4 space-y-4 bg-white dark:bg-[#282828]">
                                                            {(() => {
                                                                ...
                                                                if (!txt) return null;
                                                                return <div><h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Explanation</h4><RichQuestionContent content={txt} compact /></div>;
                                                            })()}
                                                            {(() => {
                                                            ...
                                                            {solution.bruteForce.code && Object.keys(solution.bruteForce.code).length > 0 && (() => {
                                                                ...
                                                                const selLang = selectedLanguage.bruteForce || getDefaultSolutionCodeLanguage(langs);
                                                    ...
                                                    {expandedSolution === "optimized" && (
                                                        <div className="p-4 space-y-4 bg-white dark:bg-[#282828]">
                                                            ...
                                                            {solution.optimized.code && Object.keys(solution.optimized.code).length > 0 && (() => {
                                                                ...
                                                                if (!langs.length) return null;
                                                                const selLang = selectedLanguage.optimized || getDefaultSolutionCodeLanguage(langs);
                                        ...
                                        <>
                                            ...
                                            {testPanelTab === "result" && hasTestRun && (
                                                <div className="mb-4 flex items-baseline gap-4">
                                                    <h2 className={`text-2xl font-semibold ${
                                                        ...
                                                        Object.values(results).some(isFailedResult) ? "text-red-500" :
                                                        Object.values(results).every(isAcceptedResult) ? "text-green-500" :
                                                        "text-slate-500"
                                                    }`}>
                                                        {isRunning || isSubmitting ? "Running..." :
                                                         ...
                                                         submissionResult?.status === "wrong_answer" ? (Object.values(results).find(isFailedResult)?.status || "Wrong Answer") :
                                                         Object.values(results).every(isAcceptedResult) && Object.keys(results).length > 0 ? "Accepted" :
                                                         Object.values(results).find(isFailedResult)?.status || (Object.keys(results).length > 0 ? "Wrong Answer" : "Ready")}
                                                    </h2>
                                                </div>
                                            )}
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-4">
                                                {tests.map((t, i) => {
                                                    ...
                                                    const isPass = isAcceptedResult(res);
                                                    const isFail = isFailedResult(res);
                                                    ...
                                                    return (
                                                        <button key={i} onClick={() => setActiveTestCaseIndex(i)}
                                                            ...
                                                            {res?.type === "hidden" ? "Hidden" : "Case"} {res?.type === "hidden" ? i - sampleTests.length + 1 : i + 1}
                                                        </button>
                                                    );
                                                })}
                                            </div>
                                                        ...
                                                        {testPanelTab === "result" && isFinished && (
                                                            <>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="p-4 rounded-lg bg-slate-50 dark:bg-[#282828]">
                                                                        <code className="font-mono text-sm whitespace-pre-wrap break-words block text-slate-700 dark:text-slate-300">{getResultOutput(res)}</code>
                                                                    </div>
                                                                </div>
                                                                <div>
                                                                ...
                                                                {(res.runtime !== undefined || res.memory !== undefined) && (
                                                                    <div className="flex flex-wrap gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                                                                        {res.runtime !== undefined && (
                                                                            <span className="rounded-full bg-slate-100 px-3 py-1 dark:bg-[#282828]">CPU {res.runtime} ms</span>
                                                                        )}
                                                                        {res.memory !== undefined && (
                                                                            <span className="rounded-full bg-slate-100 px-3 py-1 dark:bg-[#282828]">
                                                                                Memory {typeof res.memory === "number" ? `${(res.memory / 1024).toFixed(1)} MB` : String(res.memory)}
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                )}
                            ...
                            {activeTab === "description" ? (
                                <div className="prose dark:prose-invert max-w-none">
                                    ...
                                    <div className="whitespace-pre-wrap dark:text-slate-300">
                                        <RichQuestionContent content={q.description || q.statement || ""} />
                                    </div>
                                    {q.schema && (
                                        <>
                                    ...
                                    {q.examples && q.examples.length > 0 && (
                                        <>
                                            ...
                                            <div className="bg-slate-50 dark:bg-[#1c160d] p-4 rounded space-y-4">
                                                ...
                                                {q.examples[0].explanation && <><p className="font-semibold mt-4 mb-2">Explanation:</p><RichQuestionContent content={q.examples[0].explanation} compact /></>}
                                            </div>
                                        </>
                                    )}
                                </div>
                                        ...
                                        <section>
                                            ...
                                            <ol className="space-y-2 list-decimal list-inside marker:text-slate-400">
                                                {q.followUpQuestions.map((fq: string, i: number) => (
                                                    <li key={i} className="pl-1 text-[15.5px] leading-relaxed text-slate-700 dark:text-slate-200">
                                                        <RichQuestionContent content={fq} compact />
                                                    </li>
                                                ))}
                                            </ol>
                                        </section>
                                                ...
                                                <div className="space-y-3">
                                                    {hints.slice(0, revealedHints).map((hint: string, i: number) => (
                                                        <div key={i} className="flex items-start gap-3">
                                                            ...
                                                            <div className="min-w-0 flex-1 text-[15.5px] text-slate-700 dark:text-slate-200">
                                                                <RichQuestionContent content={hint} compact />
                                                            </div>
                                                        </div>
                                                    ))}
